      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        <MotionContainer className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="space-y-1">
            ...
            <h1 className="text-3xl font-display font-bold text-slate-900">Student Consultations</h1>
            <p className="text-slate-500 text-sm">View and manage all student appointment requests</p>
          </div>
        ...
        <MotionContainer delay={0.05} className="grid grid-cols-2 sm:grid-cols-5 gap-3">
          <SpotlightCard spotlightColor="rgba(153, 0, 0, 0.06)" className="p-4">
            <p className="text-xs text-slate-500 font-medium">Total</p>
            <p className="text-2xl font-extrabold text-slate-900">{stats.total}</p>
          </SpotlightCard>
          <SpotlightCard 
            spotlightColor="rgba(245, 158, 11, 0.06)" 
            className={`p-4 cursor-pointer hover:border-amber-500/30 transition-colors ${filter === 'pending' ? 'border-amber-500/30' : ''}`}
          ...
          <SpotlightCard 
            ...
            className={`p-4 cursor-pointer hover:border-emerald-500/30 transition-colors ${filter === 'confirmed' ? 'border-emerald-500/30' : ''}`}
          ...
          <SpotlightCard 
            ...
            className={`p-4 cursor-pointer hover:border-purple-500/30 transition-colors ${filter === 'completed' ? 'border-purple-500/30' : ''}`}
            onClick={() => setFilter('completed')}
          >
            <p className="text-xs text-slate-500 font-medium">Completed</p>
            <p className="text-2xl font-extrabold text-purple-600">{stats.completed}</p>
          </SpotlightCard>
          <SpotlightCard 
            spotlightColor="rgba(244, 63, 94, 0.06)" 
            className={`p-4 cursor-pointer hover:border-red-500/30 transition-colors ${filter === 'cancelled' ? 'border-red-500/30' : ''}`}
        ...
        <MotionContainer delay={0.1}>
          <div className="flex gap-2 flex-wrap bg-white p-2 rounded-2xl border border-primary-500/10 shadow-card">
            {filterTabs.map((tab) => {
        ...
        {loading ? (
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="space-y-3 flex-grow">
                    ...
                    <p className="text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <strong className="text-slate-700">Meeting Purpose:</strong> "{appointment.purpose || 'Not specified'}"
                    </p>
                  </div>

                  <div className="flex flex-col items-end gap-4 border-t md:border-t-0 border-slate-200 pt-3 md:pt-0 shrink-0">
          ...
          <MotionContainer delay={0.2}>
            <div className="bg-white rounded-3xl shadow-card border border-primary-500/10 p-12 text-center space-y-4">
              {filter === 'pending' ? (
                <>
                  <CheckCircle2 className="w-12 h-12 text-emerald-500 mx-auto opacity-80" />
                  <h3 className="text-lg font-display font-bold text-slate-900">All Clear!</h3>
                  <p className="text-sm text-slate-500 max-w-sm mx-auto">
                    You have no pending appointment requests. Great job keeping up with your schedule!
                  </p>
                  <button
                    onClick={() => setFilter('all')}
                    className="text-xs text-primary-500 hover:text-primary-600 font-semibold inline-flex items-center gap-1"
                  >
                ...
                <>
                  ...
                  <h3 className="text-lg font-display font-bold text-slate-900">No Confirmed Appointments</h3>
                  <p className="text-sm text-slate-500 max-w-sm mx-auto">
                ...
                <>
                  ...
                  <h3 className="text-lg font-display font-bold text-slate-900">No Completed Appointments</h3>
                  <p className="text-sm text-slate-500 max-w-sm mx-auto">
                ...
                <>
                  ...
                  <h3 className="text-lg font-display font-bold text-slate-900">No Cancelled Appointments</h3>
                  <p className="text-sm text-slate-500 max-w-sm mx-auto">
                ...
                <>
                  <Calendar className="w-12 h-12 text-slate-400 mx-auto" />
                  <h3 className="text-lg font-display font-bold text-slate-900">No Consultations Found</h3>
                  <p className="text-sm text-slate-500 max-w-sm mx-auto">
