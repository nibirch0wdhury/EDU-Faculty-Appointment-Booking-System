  ...
  return (
    <PageTransition className="py-8 md:py-12 bg-slate-50/80 pattern-dots">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        {/* Error Message */}
        {error && (
          <MotionContainer className="p-4 bg-red-50 border border-red-200 rounded-2xl text-red-600 text-sm flex items-center gap-3">
        ...
        <MotionContainer delay={0.1} className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <SpotlightCard spotlightColor="rgba(153, 0, 0, 0.06)" className="p-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-xs uppercase font-bold tracking-wider text-slate-500">Total Bookings</p>
          ...
          <SpotlightCard spotlightColor="rgba(16, 185, 129, 0.06)" className="p-6">
            <div className="flex items-center justify-between">
              <div className="space-y-1">
                <p className="text-xs uppercase font-bold tracking-wider text-slate-500">Upcoming Consultations</p>
                <p className="text-3xl font-extrabold text-emerald-600">{stats.upcoming}</p>
              </div>
        ...
        <MotionContainer delay={0.3} className="bg-white rounded-3xl shadow-card border border-primary-500/10 p-6 sm:p-8 space-y-6">
          ...
          <div className="flex items-center justify-between border-b border-slate-200 pb-4">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-xl font-display font-bold text-slate-900">Upcoming Consultations</h2>
            </div>
            <Link to="/student/my-appointments" className="text-xs font-semibold text-primary-500 hover:text-primary-600 flex items-center gap-1">
              <span>View All</span>
          ...
          {loading ? (
            ...
            <div className="space-y-3">
              {upcomingList.slice(0, 4).map((appointment) => (
                <div key={appointment._id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-4 rounded-xl bg-slate-50 border border-slate-200 gap-4 hover:border-primary-500/30 transition-colors">
                  <div className="space-y-1">
                    <p className="font-bold text-slate-900">{appointment.facultyId?.name || 'Faculty Member'}</p>
                    ...
                    {appointment.purpose && (
                      <p className="text-xs text-slate-500 italic">"{appointment.purpose}"</p>
                    )}
                  </div>
            ...
            <div className="py-12 text-center text-slate-500 space-y-3">
              ...
              <Link to="/student/book-appointment">
                <span className="inline-flex items-center gap-1 text-sm text-primary-500 font-semibold hover:underline">
