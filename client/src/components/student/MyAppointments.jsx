  ...
  return (
    <PageTransition className="py-8 md:py-12 bg-slate-50/80 pattern-dots">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
        ...
        {/* Filter Tabs */}
        <MotionContainer delay={0.1} className="flex gap-2 flex-wrap bg-white p-2 rounded-2xl border border-primary-500/10 shadow-card">
        ...
        {loading ? (
          ...
          <MotionContainer delay={0.2} className="space-y-4">
            {filteredAppointments.map((appointment) => (
              <SpotlightCard key={appointment._id} spotlightColor="rgba(153, 0, 0, 0.06)" className="p-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                  <div className="space-y-3 flex-grow">
                    ...
                    <p className="text-xs text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-200">
                      <strong className="text-slate-700">Purpose:</strong> {appointment.purpose}
                    </p>
                  </div>

                  <div className="flex flex-row md:flex-col items-center md:items-end justify-between gap-4 border-t md:border-t-0 border-slate-200 pt-3 md:pt-0">
                    <Badge status={appointment.status} />

                    {appointment.status !== 'cancelled' && appointment.status !== 'completed' && (
                      <button
                        onClick={() => handleCancel(appointment._id)}
                        className="text-xs font-semibold text-red-500 hover:text-red-600 flex items-center gap-1 hover:underline transition-all"
                      >
          ...
          <MotionContainer delay={0.2} className="bg-white rounded-3xl shadow-card border border-primary-500/10 p-12 text-center space-y-4">
            ...
            <h3 className="text-lg font-display font-bold text-slate-900">No Appointments Found</h3>
